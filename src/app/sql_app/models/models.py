...
from app.sql_app.models.enums import Currency, IntervalType, Status
...
class User(Base):
    ...
    recurring_transactions = relationship(
        ...
        back_populates="user",
        foreign_keys="[RecurringTransaction.user_id]",
    )
    recurring_received_transactions = relationship(
        "RecurringTransaction",
        back_populates="recipient",
        foreign_keys="[RecurringTransaction.recipient_id]",
    )
...
class Card(Base):
    ...
    transactions = relationship(
        "Transaction", back_populates="card", foreign_keys="[Transaction.card_id]"
    )
    recurring_transactions = relationship(
        "RecurringTransaction",
        foreign_keys="[RecurringTransaction.card_id]",
        back_populates="card",
    )
...
class Transaction(Base):
    ...
    recipient = relationship(
        "User", back_populates="received_transactions", foreign_keys=[recipient_id]
    )
...
class Contact(Base):
    ...
    user_contact = relationship(
        "User", back_populates="contacts", foreign_keys=[user_contact_id]
    )
...
class RecurringTransaction(Base):
    ...
    user = relationship(
        "User", back_populates="recurring_transactions", foreign_keys=[user_id]
    )
    card = relationship(
        "Card", back_populates="recurring_transactions", foreign_keys=[card_id]
    )
    recipient = relationship(
        "User",
        back_populates="recurring_received_transactions",
...
class Wallet(Base):
    ...
    transactions = relationship(
        "Transaction", back_populates="wallet", foreign_keys="[Transaction.wallet_id]"
    )
