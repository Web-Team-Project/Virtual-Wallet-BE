...
async def test_create_wallet_already_exists(db, mock_user):
    db.execute.side_effect = [
        MagicMock(scalars=MagicMock(return_value=MagicMock(first=MagicMock(return_value=mock_user)))),  # User found
        MagicMock(scalars=MagicMock(return_value=MagicMock(first=MagicMock(return_value=MagicMock(spec=Wallet)))))  # Wallet found
    ]
